        ...
        <div style={{ maxWidth: 1300, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 0 0" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              {tab === "todo" && (
                <button onClick={() => exportToWord(tasks, extraNames)}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: K.white, borderRadius: 14, padding: 28, width: "100%", maxWidth: 460, boxShadow: "0 24px 60px rgba(0,0,0,0.18)" }}>
            {modal === "task" && (
              <>
                ...
                <Field label="Titlu">
                  <input value={taskForm.title} onChange={(e) => setTaskForm((f) => ({ ...f, title: e.target.value }))} placeholder="Ex: Redactat memo salarizare" style={iStyle} />
                </Field>
                <Field label="Coloana">
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {COLUMNS.map((col) => {
                      const pc = col === "BD" ? { bg: K.purple, pale: "#f5eef3" } : getPersonColor(col, extraNames);
                      const active = taskForm.column === col;
                      return (
                        <button key={col} onClick={() => setTaskForm((f) => ({ ...f, column: col }))}
                          style={{ padding: "7px 12px", borderRadius: 7, border: `1.5px solid ${active ? pc.bg : K.gray10}`, background: active ? pc.pale : K.white, color: active ? pc.bg : K.gray50, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>{col}</button>
                      );
                    })}
                  </div>
                </Field>
                <Field label="Asigneaza">
...
function TaskCard({ task, col, isBD, color, extraNames, onOpen, onToggleImportant, onToggleDone, onDelete }) {
  ...
  const delegatorColor = task.column && !isBD ? getPersonColor(task.column, extraNames).bg : color;
  return (
    <div onClick={() => onOpen(task)} style={{ background: done ? K.grayBg : K.white, border: `1.5px solid ${overdue ? "#e53e3e" : K.gray10}`, borderLeft: `4px solid ${overdue ? "#e53e3e" : done ? K.gray20 : color}`, borderRadius: 10, padding: "12px 12px 10px", cursor: "pointer", position: "relative", opacity: done ? 0.75 : 1 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {isDelegated && <div style={{ fontSize: 9, fontWeight: 700, color: delegatorColor, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>↳ de la {task.column}</div>}
