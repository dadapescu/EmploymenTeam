...
function Planner({ me, onSwitch }) {
  ...
  function isOnLeaveToday(person) {
    const todayIso = new Date().toISOString().slice(0, 10);
    return leaves.some((l) => l.person === person && l.start <= todayIso && todayIso <= l.end);
  }
        ...
        <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 24px 40px", overflowX: "auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${COLUMNS.length}, minmax(220px, 1fr))`, gap: 16, alignItems: "start" }}>
            {COLUMNS.map((col) => {
              ...
              const isBD = col === "BD";
              const isMe = col === me;
              const onLeave = !isBD && isOnLeaveToday(col);
                ...
                <div key={col}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 700, color: K.gray70, lineHeight: 1 }}>{col === "BD" ? "BD / NB" : col}{isMe ? " (tu)" : ""}</span>
                      ...
                      {onLeave && <span style={{ fontSize: 13, fontWeight: 700, color: K.gray70, display: "inline-flex", alignItems: "center", lineHeight: 1 }}>🌴 Concediu</span>}
                    </div>
