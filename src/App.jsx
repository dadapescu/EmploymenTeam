        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {TEAM.map((name) => (
            <button key={name} onClick={() => onPick(name)}
              style={{ padding: "13px 0", borderRadius: 10, border: `2px solid ${PERSON_COLORS[name].bg}`, background: PERSON_COLORS[name].pale, color: PERSON_COLORS[name].bg, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
              {name}
            </button>
          ))}
        </div>
...
function Planner({ me, onSwitch }) {
  ...
  // Columns ordered so "me" comes first
  ...
  const emptyTask = { title: "", assignees: [], due: "", important: false, column: me, done: false };
  ...
  function tasksForColumn(col) {
    if (col === "BD") return tasks.filter((t) => t.column === "BD");
    return tasks.filter((t) => t.column === col || (t.assignees?.includes(col) && t.column !== col && t.column !== "BD"));
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: K.grayBg, color: K.gray70 }}>
      {/* Header */}
      <div style={{ background: K.gray70, color: K.white }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 0 0" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              {tab === "todo" && (
                <>
                  <button onClick={() => exportToWord(tasks)}
                    title="Descarca Word cu taskurile pe persoane"
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: 8 }}>
                <span style={{ background: PERSON_COLORS[me].bg, color: "#fff", borderRadius: 6, padding: "5px 10px", fontSize: 12, fontWeight: 700 }}>{me}</span>
                <button onClick={onSwitch} title="Schimba utilizator"
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 24px 40px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, alignItems: "start" }}>
            {COLUMNS.map((col) => {
              ...
              const isBD = col === "BD";
              const isMe = col === me;
              const color = isBD ? K.purple : PERSON_COLORS[col].bg;
              return (
                <div key={col}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 10, height: 10, borderRadius: "50%", background: color }} />
                      <span style={{ fontSize: 13, fontWeight: 700, color: K.gray70, letterSpacing: 0.3 }}>{col}{isMe ? " (tu)" : ""}</span>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    {active.map((t) => (
                      <TaskCard key={t.id + col} task={t} col={col} isBD={isBD} color={color} onOpen={openEditTask} onToggleImportant={toggleImportant} onToggleDone={toggleDone} onDelete={deleteTask} />
                    ))}
                  </div>
                    ...
                    <div style={{ marginTop: 14 }}>
                      ...
                      {expandedDone[col] && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 6 }}>
                          {done.map((t) => (
                            <TaskCard key={t.id + col + "d"} task={t} col={col} isBD={isBD} color={color} onOpen={openEditTask} onToggleImportant={toggleImportant} onToggleDone={toggleDone} onDelete={deleteTask} />
                          ))}
                        </div>
                      )}
                    </div>
      ...
      {tab === "leaves" && <LeaveTab leaves={leaves} onDelete={deleteLeave} onEdit={(l) => { setLeaveForm({ ...l }); setEditId(l.id); setModal("leave"); }} />}
      ...
      {modal && (
        <div onClick={() => setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(83,86,90,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: K.white, borderRadius: 14, padding: 28, width: "100%", maxWidth: 440, boxShadow: "0 24px 60px rgba(0,0,0,0.18)" }}>
            {modal === "task" && (
              <>
                ...
                <Field label="Titlu *"><input value={taskForm.title} onChange={(e) => setTaskForm((f) => ({ ...f, title: e.target.value }))} placeholder="Ex: Redactat memo salarizare" style={iStyle} /></Field>
                <Field label="Coloana principala">
                  <div style={{ display: "flex", gap: 6 }}>
                    {COLUMNS.map((col) => (
                      <button key={col} onClick={() => setTaskForm((f) => ({ ...f, column: col }))}
                        style={{ flex: 1, padding: "7px 4px", borderRadius: 7, border: `1.5px solid ${taskForm.column === col ? (col === "BD" ? K.purple : PERSON_COLORS[col].bg) : K.gray10}`, background: taskForm.column === col ? (col === "BD" ? "#f5eef3" : PERSON_COLORS[col].pale) : K.white, color: taskForm.column === col ? (col === "BD" ? K.purple : PERSON_COLORS[col].bg) : K.gray50, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>{col}</button>
                    ))}
                  </div>
                </Field>
                <Field label="Asigneaza persoane (taskul apare si la ele)">
                  <div style={{ display: "flex", gap: 8 }}>
                    {TEAM.map((name) => (
                      <button key={name} onClick={() => toggleAssignee(name)}
                        style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: `1.5px solid ${taskForm.assignees.includes(name) ? PERSON_COLORS[name].bg : K.gray10}`, background: taskForm.assignees.includes(name) ? PERSON_COLORS[name].pale : K.white, color: taskForm.assignees.includes(name) ? PERSON_COLORS[name].bg : K.gray50, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{name}</button>
                    ))}
                  </div>
                </Field>
                <Field label="Termen"><input type="date" value={taskForm.due} onChange={(e) => setTaskForm((f) => ({ ...f, due: e.target.value }))} style={iStyle} /></Field>
                <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
                  <button onClick={() => setTaskForm((f) => ({ ...f, important: !f.important }))} style={{ background: "none", border: `1.5px solid ${taskForm.important ? K.orange : K.gray10}`, borderRadius: 7, padding: "7px 14px", cursor: "pointer", color: taskForm.important ? K.orange : K.gray30, fontSize: 13, fontWeight: 600 }}>★ {taskForm.important ? "Important" : "Marcheaza important"}</button>
                  <button onClick={() => setTaskForm((f) => ({ ...f, done: !f.done }))} style={{ background: taskForm.done ? K.gray70 : "none", border: `1.5px solid ${taskForm.done ? K.gray70 : K.gray10}`, borderRadius: 7, padding: "7px 14px", cursor: "pointer", color: taskForm.done ? "#fff" : K.gray30, fontSize: 13, fontWeight: 600 }}>✓ Done</button>
                </div>
            ...
            {modal === "leave" && (
              <>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    {TEAM.map((name) => (
                      <button key={name} onClick={() => setLeaveForm((f) => ({ ...f, person: name }))}
                        style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: `1.5px solid ${leaveForm.person === name ? PERSON_COLORS[name].bg : K.gray10}`, background: leaveForm.person === name ? PERSON_COLORS[name].pale : K.white, color: leaveForm.person === name ? PERSON_COLORS[name].bg : K.gray50, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{name}</button>
                    ))}
                  </div>
                </Field>
                ...
                <Field label="Nota (optional)"><input value={leaveForm.label} onChange={(e) => setLeaveForm((f) => ({ ...f, label: e.target.value }))} placeholder="Ex: Concediu vara" style={iStyle} /></Field>
...
// ── Task card ─────────────────────────────────────────────────────
function TaskCard({ task, col, isBD, color, onOpen, onToggleImportant, onToggleDone, onDelete }) {
  const overdue = !task.done && isOverdue(task.due);
  const isDelegated = !isBD && task.column !== col && task.assignees?.includes(col);
  ...
  return (
    <div onClick={() => onOpen(task)} style={{ background: done ? K.grayBg : K.white, border: `1.5px solid ${overdue ? "#e53e3e" : K.gray10}`, borderLeft: `4px solid ${overdue ? "#e53e3e" : done ? K.gray20 : color}`, borderRadius: 10, padding: "12px 12px 10px", cursor: "pointer", position: "relative", opacity: done ? 0.75 : 1 }}>
      <button onClick={(e) => { e.stopPropagation(); onToggleImportant(task.id); }} title="Important" style={{ position: "absolute", top: 8, right: 8, background: "none", border: "none", cursor: "pointer", fontSize: 14, color: task.important ? K.orange : K.gray20, padding: 0 }}>★</button>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
        <button onClick={(e) => { e.stopPropagation(); onToggleDone(task.id); }} title={done ? "Nefinalizat" : "Done"} style={{ flexShrink: 0, marginTop: 1, width: 18, height: 18, borderRadius: 5, border: `2px solid ${done ? color : K.gray20}`, background: done ? color : "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, fontWeight: 700, padding: 0 }}>{done ? "✓" : ""}</button>
        <div style={{ flex: 1, minWidth: 0 }}>
          {isDelegated && <div style={{ fontSize: 9, fontWeight: 700, color: PERSON_COLORS[task.column]?.bg || K.gray50, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>↳ de la {task.column}</div>}
          <div style={{ fontSize: 13, fontWeight: 600, color: overdue ? "#e53e3e" : K.gray70, lineHeight: 1.4, paddingRight: 18, marginBottom: 8, textDecoration: done ? "line-through" : "none" }}>{task.title}</div>
          {task.assignees?.length > 0 && (
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 6 }}>
              {task.assignees.map((a) => <span key={a} style={{ background: PERSON_COLORS[a]?.pale || K.orangePale, color: PERSON_COLORS[a]?.bg || K.orange, borderRadius: 5, padding: "2px 7px", fontSize: 10, fontWeight: 700 }}>{a}</span>)}
            </div>
          )}
...
// ── Leave calendar ────────────────────────────────────────────────
function LeaveTab({ leaves, onDelete, onEdit }) {
  const months = [];
  for (let y = 2026; y <= 2027; y++) { const start = y === 2026 ? 6 : 0; for (let m = start; m < 12; m++) months.push({ y, m }); }
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
        {TEAM.map((name) => (
          <div key={name} style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ width: 12, height: 12, borderRadius: 3, background: PERSON_COLORS[name].bg }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: K.gray70 }}>{name}</span>
          </div>
        ))}
        <div style={{ marginLeft: "auto", fontSize: 12, color: K.gray30 }}>{leaves.length} {leaves.length === 1 ? "concediu" : "concedii"}</div>
      </div>

      {leaves.length > 0 && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 28 }}>
          {[...leaves].sort((a, b) => a.start.localeCompare(b.start)).map((l) => (
            <div key={l.id} onClick={() => onEdit(l)} style={{ background: PERSON_COLORS[l.person].pale, border: `1.5px solid ${PERSON_COLORS[l.person].bg}`, borderRadius: 8, padding: "6px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: PERSON_COLORS[l.person].bg }} />
              <span style={{ fontSize: 12, fontWeight: 600, color: PERSON_COLORS[l.person].bg }}>{l.person}</span>
            ...
            <div key={`${y}-${m}`} style={{ background: K.white, borderRadius: 12, padding: "16px 14px", border: `1px solid ${K.gray10}` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2 }}>
                ...
                {cells.map((d, i) => {
                  ...
                  const bgs = dl.map((l) => PERSON_COLORS[l.person]?.bg).filter(Boolean);
